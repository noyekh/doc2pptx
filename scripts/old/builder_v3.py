...
class PPTBuilder:
    ...
    def _fill_table_slide(self, pptx_slide: PptxSlide, slide: Slide) -> None:
        ...
        for block in slide.blocks:
            if block.content.content_type == ContentType.TABLE:
                table_block = block
                break
        
        if not table_block or not table_block.content.table:
        ...
        try:
            ...
            # NO FALLBACK text box creation - we don't want the overlay
            # Instead, log a message and continue
            logger.error("Skipping fallback text box to prevent overlay issues")
    ...
    def _add_bullet_points_to_placeholder(
        ...
        for i, bullet_text in enumerate(bullet_points):
            ...
            if as_bullets:
                p.level = 0          # active la liste
                self._ensure_bullet_visible(p)
                if as_bullets and not p.text.lstrip().startswith("•"):
                    if p.runs:
                        p.runs[0].text = f"• {p.runs[0].text}"
                    else:
                        p.text = f"• {p.text}"
            else:
                self._remove_bullet(p)      # paragraphe normal
    ...
    def _add_table_to_slide(self, pptx_slide: PptxSlide, headers: List[str], 
        ...
        try:
            ...
            # Calculate table position and size
            # Typically below the title and centered in the slide
            title_height = Inches(1.5)  # Approximate title area height
            left = Inches(0.5)
            top = title_height + Inches(0.5)  # Add some margin
            width = Inches(9)  # Standard slide width minus margins
            height = Inches(5)  # Adjust based on table content
            
            # Create the table shape
            left, top  = self._emu(left), self._emu(top)
            width, height = self._emu(width), self._emu(height)
            table = pptx_slide.shapes.add_table(num_rows, num_cols, left, top, width, height).table
    ...
    def _apply_table_style(self, table: Table, style_preset: Dict[str, Any]) -> None:
        ...
        if len(table.rows) > 0:
            table.rows[0].height = self._emu(row_height * 1.2)  # 1ʳᵉ ligne légèrement plus haute
        for r in table.rows[1:]:
            r.height = self._emu(row_height)
        ...
        _has_border_api = hasattr(table.cell(0, 0), "border_top")
        if _has_border_api and border_color:
            for row in table.rows:
                for cell in row.cells:
                    for side in ("border_top", "border_bottom", "border_left", "border_right"):
                        border = getattr(cell, side)
                        border.color.rgb = self._hex_to_rgb(border_color)
